...
class Warehouse:
    ...
    def add_product(self, product_name: str, amount: int = 0, limit: int = 10) -> None:
        with open(json_path) as f:
            ...
            if product_name not in json_content:
                raise Exception("Product doesn't exists (not specified in product register)")
            elif product_name in self.__products.keys():
                raise Exception("Product already exists in warehouse!")
            elif amount < 0:
                raise ValueError("Amount cannot be less than zero!")
            else:
                self.__products[product_name] = Product(product_name, amount, limit)
    ...
    def increase_amount(self, product_name: str, amount: int = 1) -> None:
        if product_name not in self.__products:
            raise Exception("Product doesn't exists in warehouse!")
        if (self.__products[product_name].amount + amount) < self.__products[product_name].limit:
            ...
            raise AttributeError(f"Cannot have more products than specified limit of {self.__products[product_name].limit}!")
